...
export const useTimeRange = (initialValue: TimeRange = 7) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(TIME_RANGE_CHANGE_EVENT, handleTimeRangeChange as EventListener);
    return () => window.removeEventListener(TIME_RANGE_CHANGE_EVENT, handleTimeRangeChange as EventListener);
  }, []);
