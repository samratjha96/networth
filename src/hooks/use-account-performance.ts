...
export function useAccountPerformance(
  ...
  const calculatePerformance = useCallback(async () => {
    ...
    try {
      console.log(
        `Calculating account performance for ${period}, backend: ${backendType}`,
      );
      ...
      // Always generate consistent simulated data for account performance
      // This ensures a uniform experience regardless of backend
      ...
      accounts.forEach((account) => {
        const baseRate =
          growthRates[account.type] || (account.isDebt ? -0.5 : 2.0);
  ...
  }, [accounts, period, db, backendType]);

  // Recalculate when dependencies change - including backend type
  useEffect(() => {
