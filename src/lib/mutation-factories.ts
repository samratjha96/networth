  ...
  queryClient.setQueriesData(
    ...
    (old: any) => {
      ...
      return {
        ...
        percentageChange: old.previousValue && old.previousValue !== 0
          ? ((newNetWorth - old.previousValue) / Math.abs(old.previousValue)) * 100
          : 0,
      };
    }
  );
 ...
 */
export const createOptimisticAddMutation = <TData extends { id: string }, TInput = Omit<TData, 'id'>>(
  queryClient: QueryClient,
  ...
  options: {
    ...
    updateRelatedQueries?: (queryClient: QueryClient, newItem: TData, allItems: TData[]) => void;
  }
) => {
  return useMutation(createMutationOptions<TData, Error, TInput, { previous: TData[] | undefined; optimistic: TData }>({
    mutationFn,
    ...
    onError: (error, variables, context) => {
      ...
      if (context?.previous) {
        ...
        if (context.previous.length > 0) {
          options.updateRelatedQueries?.(queryClient, context.optimistic, context.previous);
        }
      }
    },
  }));
...
export const createOptimisticUpdateMutation = <TData extends { id: string }>(
  ...
  options: {
    userId: string | null;
    queryKey: readonly unknown[];
    updateRelatedQueries?: (queryClient: QueryClient, updatedItem: TData, allItems: TData[]) => void;
  }
) => {
  return useMutation(createMutationOptions<void | TData, Error, TData, { previous: TData[] | undefined }>({
    mutationFn,
    ...
    onError: (error, variables, context) => {
      ...
      if (context?.previous) {
        ...
        if (context.previous.length > 0) {
          options.updateRelatedQueries?.(queryClient, variables, context.previous);
        }
      }
    },
  }));
...
export const createOptimisticDeleteMutation = <TData extends { id: string }>(
  ...
  options: {
    userId: string | null;
    queryKey: readonly unknown[];
    updateRelatedQueries?: (queryClient: QueryClient, deletedId: string, remainingItems: TData[]) => void;
  }
) => {
  return useMutation(createMutationOptions<void, Error, string, { previous: TData[] | undefined; deleted: TData | undefined }>({
    mutationFn,
 ...
 */
export const createAccountMutations = (userId: string | null, queryClient: QueryClient) => {
  ...
  return {
    ...
    generateOptimisticAccount: (input: Omit<AccountWithValue, 'id'>): AccountWithValue => ({
      ...input,
