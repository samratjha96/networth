...
import { Database, Tables, TablesInsert, TablesUpdate, Account, AccountType, CurrencyCode, DatabaseProvider, NetworthHistory } from '@/types';
...
const TEST_USER_EMAIL = import.meta.env.VITE_SUPABASE_TEST_USER_EMAIL || "test@example.com";
const TEST_USER_PASSWORD = import.meta.env.VITE_SUPABASE_TEST_USER_PASSWORD || "testpassword123";
...
const accountToDbAccount = (account: Omit<Account, "id">, userId: string): AccountInsert => {
...
export class SupabaseDatabase implements DatabaseProvider {
  ...
  private constructor() {
    if (!supabaseUrl || !supabaseKey) {
      ...
      throw new Error('Supabase URL and key must be provided in .env file with VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY');
    }
  ...
  private async initializeTestAuth(): Promise<void> {
    ...
    if (!TEST_USER_EMAIL || !TEST_USER_PASSWORD) {
      ...
      console.error('Please add VITE_SUPABASE_TEST_USER_EMAIL and VITE_SUPABASE_TEST_USER_PASSWORD to your .env file');
      return;
    }
      ...
      if (!user) {
        ...
        if (error) {
          ...
          if (error.message.includes('Invalid login credentials')) {
            console.error('The email or password is incorrect. Verify your test user credentials in the .env file.');
          } else if (error.message.includes('Email not confirmed')) {
            console.error('Your test user email is not confirmed. Go to Supabase dashboard and confirm it manually.');
          }
          return;
        }
        
        if (data?.user) {
          console.log('✅ Successfully signed in as test user:', data.user.email);
          console.log('🆔 User ID:', data.user.id);
          this.userId = data.user.id;
          
          // Verify the user ID matches what we expect
          if (data.user.id !== TEST_USER_ID) {
            console.warn('⚠️ Warning: The authenticated user ID does not match TEST_USER_ID in your code!');
            console.warn(`Authenticated as: ${data.user.id}`);
            console.warn(`Expected: ${TEST_USER_ID}`);
            console.warn('Update TEST_USER_ID in supabase-database.ts to match the authenticated user ID.');
          }
        }
      } else {
  ...
  async updateAccount(account: Account): Promise<void> {
    ...
    const { balance, is_debt, currency, name, type } = accountToDbAccount(account, userId);
