...
const generateDailyChange = (baseValue: number, volatility: number = 0.0005) => {
...
export const generateMockNetworthHistory = (
  ...
  if (startValue === undefined || isNaN(startValue)) {
    console.error('Invalid startValue in generateMockNetworthHistory:', startValue);
    return [];
  }

  console.log(`[HISTORY] Generating history with startValue: ${startValue}, trend: ${trend}`);
  ...
  for (let i = 1; i <= days; i++) {
    ...
    const seasonalChange = (lastMonthValue * seasonalFactor * monthProgress) / 20; // Increased impact
    
    // Daily volatility
    const dailyChange = generateDailyChange(currentValue, volatility);
    
    // Combine changes with weighted importance
    const totalDailyChange = (trendChange * 1.5) + (seasonalChange * 2) + dailyChange;
    const proposedValue = currentValue + totalDailyChange;
    
    // Calculate percentage change for logging
    const percentChange = ((proposedValue - currentValue) / Math.abs(currentValue)) * 100;
    ...
    if (Math.abs(percentChange) > 2) {
      ...
      console.log(`  Components: trend=${trendChange.toFixed(2)}, seasonal=${seasonalChange.toFixed(2)}, daily=${dailyChange.toFixed(2)}`);
    }
    ...
    if (i > 1 && date.getDate() === 1) {
      const monthlyChange = ((currentValue - lastMonthValue) / Math.abs(lastMonthValue)) * 100;
      console.log(`[HISTORY] Month transition to ${date.toLocaleString('default', { month: 'long' })}`);
  ...
  const totalChange = ((currentValue - startValue) / Math.abs(startValue)) * 100;
...
export const generatePositiveTrendData = () => {
  ...
  const startValue = accounts.reduce((sum, account) => sum + account.balance, 0);
  if (startValue === undefined || isNaN(startValue)) {
