...
class DatabaseExplorer {
        ...
        try {
            ...
            for (const account of accounts) {
                try {
                    ...
                    const value = account.is_debt ? -Math.abs(latestValue.value) : latestValue.value;
                    totalNetWorth += value;
            ...
            accountDetails.forEach((account, index) => {
                ...
                console.log(`  ${index + 1}. ${status} ${account.name} (${account.type})${debtLabel}`);
    ...
    async analyzeNetWorthHistory(userId) {
        ...
        try {
            ...
            // Analysis
            const zeroEntries = allHistory.filter(entry => Math.abs(entry.value) < 1);
            ...
            allHistory.slice(0, 10).forEach((entry, index) => {
                ...
                console.log(`   ${index + 1}. ${date.toLocaleString()} - $${entry.value.toLocaleString()}${marker}${flag}`);
            });
            ...
            if (zeroEntries.length > 0) {
                ...
                zeroEntries.slice(0, 5).forEach(entry => {
                    console.log(`   ${new Date(entry.date).toLocaleString()} - $${entry.value}`);
                });
    ...
    async calculateExpectedNetWorth(userId) {
        ...
        if (accountAnalysis && networthAnalysis) {
            ...
            console.log(`💰 Expected (from accounts): $${expectedNetWorth.toLocaleString()}`);
            console.log(`📊 Latest history entry: $${latestHistoryValue.toLocaleString()}`);
            
            if (Math.abs(difference) > 1) {
                console.log(`⚠️  Difference: $${difference.toLocaleString()}`);
                console.log('   Net worth history may be out of sync with account values');
            } else {
    ...
    async identifyDataIssues(userId) {
        ...
        try {
            ...
            const duplicateDates = Object.entries(dateGroups).filter(([_, entries]) => entries.length > 1);
            if (duplicateDates.length > 0) {
                issues.push(`${duplicateDates.length} dates with multiple net worth entries`);
            }
    ...
    async cleanupNearZeroEntries(userId) {
        ...
        try {
            ...
            const nearZeroEntries = allEntries.filter(entry => Math.abs(entry.value) < 1);
            
            console.log(`Found ${nearZeroEntries.length} entries with near-zero values (< $1)`);
            ...
            console.log(`✅ Deleted ${deletedCount} out of ${nearZeroEntries.length} near-zero entries`);
    ...
    async createCorrectNetWorthEntry(userId) {
        ...
        try {
            ...
            console.log(`💰 Calculated net worth: $${totalNetWorth.toLocaleString()}`);
            ...
            console.log(`✅ Created new net worth entry: $${totalNetWorth.toLocaleString()}`);
